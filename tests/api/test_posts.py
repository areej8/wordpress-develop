...
def test_update_post(base_url, auth_header):
    ...
    response = requests.post(f"{base_url}/posts/{post_id}", json=data, headers=auth_header)
...
def test_delete_post(base_url, auth_header):
    ...
    post_id = posts[0]["id"]
    response = requests.delete(f"{base_url}/posts/{post_id}?force=true", headers=auth_header)
    assert response.status_code == 200
