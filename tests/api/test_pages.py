...
def test_create_page(base_url, auth_header):
    ...
    response = requests.post(f"{base_url}/pages", json=data, headers=auth_header)
...
def test_update_page(base_url, auth_header):
    ...
    pages = requests.get(f"{base_url}/pages", headers=auth_header).json()
    if not pages:
    ...
    response = requests.post(f"{base_url}/pages/{page_id}", json=update_data, headers=auth_header)
...
def test_delete_page(base_url, auth_header):
    ...
    response = requests.delete(f"{base_url}/pages/{page_id}?force=true", headers=auth_header)
    
    # Check for successful deletion
    assert response.status_code == 200
    
    # Optional: Verify it's truly gone
    verify_response = requests.get(f"{base_url}/pages/{page_id}", headers=auth_header)
