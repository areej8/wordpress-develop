...
def test_create_tag(base_url, auth_header):
    ...
    response = requests.post(f"{base_url}/tags", json=data, headers=auth_header)
    assert response.status_code == 201
    json_data = response.json()
    assert json_data["name"] == unique_name
    
    # Cleanup: Delete the created tag immediately after the test
    requests.delete(f"{base_url}/tags/{json_data['id']}?force=true", headers=auth_header)
...
def test_update_tag(base_url, auth_header):
    ...
    create_response = requests.post(f"{base_url}/tags", json=create_data, headers=auth_header)
    ...
    response = requests.post(f"{base_url}/tags/{tag_id}", json=update_data, headers=auth_header)
    ...
    # Cleanup
    requests.delete(f"{base_url}/tags/{tag_id}?force=true", headers=auth_header)
...
def test_delete_tag(base_url, auth_header):
    ...
    response = requests.delete(f"{base_url}/tags/{tag_id}?force=true", headers=auth_header)
    
    # Check for successful deletion (200 OK)
    assert response.status_code == 200
    
    # Optional: Verify it's truly gone (404 Not Found)
    verify_response = requests.get(f"{base_url}/tags/{tag_id}", headers=auth_header)
