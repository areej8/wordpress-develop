...
def test_get_comments(base_url, auth_header):
    response = requests.get(f"{base_url}/comments", headers=auth_header)
    assert response.status_code == 200
    assert isinstance(response.json(), list)

def test_create_comment(base_url, auth_header):
    # Ensure at least one post exists
    posts = requests.get(f"{base_url}/posts", headers=auth_header).json()
    if not posts:
    ...
    response = requests.post(f"{base_url}/comments", json=data, headers=auth_header)
...
def test_update_comment(base_url, auth_header):
    comments = requests.get(f"{base_url}/comments", headers=auth_header).json()
    if not comments:
        pytest.skip("No comments to update")
    comment_id = comments[0]["id"]
    data = {"content": "Updated comment via pytest"}
    response = requests.post(f"{base_url}/comments/{comment_id}", json=data, headers=auth_header)
...
def test_delete_comment(base_url, auth_header):
    ...
    comment_id = comments[0]["id"]
    response = requests.delete(f"{base_url}/comments/{comment_id}?force=true", headers=auth_header)
    assert response.status_code == 200
