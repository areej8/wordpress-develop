...
def test_create_category(base_url, auth_header):
    ...
    response = requests.post(f"{base_url}/categories", json=data, auth=auth_header)
    assert response.status_code == 201
    json_data = response.json()
    assert json_data["name"] == unique_name
    
    # Cleanup (important for taxonomy tests)
    requests.delete(f"{base_url}/categories/{json_data['id']}?force=true", auth=auth_header)
...
def test_update_category(base_url, auth_header):
    ...
    create_response = requests.post(f"{base_url}/categories", json=create_data, auth=auth_header)
    ...
    response = requests.post(f"{base_url}/categories/{category_id}", json=update_data, auth=auth_header)
    ...
    # Cleanup
    requests.delete(f"{base_url}/categories/{category_id}?force=true", auth=auth_header)
...
def test_delete_category(base_url, auth_header):
    ...
    response = requests.delete(f"{base_url}/categories/{category_id}?force=true", auth=auth_header)
