...
def test_invalid_data_type_on_update(base_url, auth_header):
    ...
    response = requests.post(f"{base_url}/posts/{post_id}", json=invalid_data, auth=auth_header)
    ...
    requests.delete(f"{base_url}/posts/{post_id}?force=true", auth=auth_header)
...
def test_nonexistent_entity_deletion(base_url, auth_header):
    ...
    response = requests.delete(f"{base_url}/posts/{non_existent_id}?force=true", auth=auth_header)
...
def test_create_published_post_by_role(base_url, request, role_fixture_name, expected_status):
    ...
    response = requests.post(f"{base_url}/posts", json=post_data, auth=auth_header)
